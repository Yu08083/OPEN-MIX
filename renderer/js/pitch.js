...
if (!_scratchCtx) {
  _scratchCtx = new (window.AudioContext || window.webkitAudioContext)();
}
return _scratchCtx;
}

export function setScratchCtx(ctx) { _scratchCtx = ctx; }
...
export async function correctPitchWithCachedCurve(buffer, curve, notes, options, progressCb) {
const { strength = 1 } = options;
const sr = buffer.sampleRate;
const numCh = buffer.numberOfChannels;
const length = buffer.length;
...
const findFreqAt = time => {
  if (!curve || curve.length === 0) return 0;
  let lo = 0, hi = curve.length - 1;
  while (lo < hi) {
    const mid = (lo + hi) >> 1;
    if (curve[mid].time < time) lo = mid + 1; else hi = mid;
  }
  return curve[lo] ? curve[lo].freq : 0;
};
...
export async function shiftPitchBySemitones(buffer, semitones, progressCb) {
const ratio = Math.pow(2, semitones / 12);
if (Math.abs(ratio - 1) < 0.0001) {
  const ctx = getScratchCtx();
  const copy = ctx.createBuffer(buffer.numberOfChannels, buffer.length, buffer.sampleRate);
  for (let c = 0; c < buffer.numberOfChannels; c++) copy.copyToChannel(buffer.getChannelData(c), c);
  if (progressCb) progressCb(1);
  return copy;
}
const sr = buffer.sampleRate;
